...
from api_client.config_loader import ConfigLoader, RunCfg
...
class HttpClient:
    ...
    def authenticate_default(self) -> HttpClient:
        self.log.info(f"Acquiring token for default user '{self.cfg.user_name}'")
        token = self._generate_token(self.cfg.user_name, self.cfg.password)
    ...
    def authenticate(self, *, username: Optional[str] = None, password: Optional[str] = None) -> str:
        user = username or self.cfg.user_name
    ...
    def request(
        ...
        *,
        ...
        url = endpoint if endpoint.startswith("http") else f"{self.base}{endpoint if endpoint.startswith('/') else '/' + endpoint}"
    ...
    def delete(self, endpoint: str, payload: Optional[Any] = None,
               headers: Optional[Mapping[str, str]] = None,
               expected_status_code: Union[int, Sequence[int], Set[int], None] = 200) -> Response:
        return self.request("DELETE", endpoint, json=payload, headers=headers, expected_status_code=expected_status_code)
