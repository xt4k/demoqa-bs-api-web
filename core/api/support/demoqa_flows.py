...
def create_demo_user(account_service: AccountService) -> UserDict:
    ...
    Returns a dict with at least: username, password, userId.
    """
    body = generate_user_request_dict()
    # Service returns parsed JSON with "userID"
...
def login_demo_user(account_service: AccountService, user: UserDict) -> UserDict:
    """
    Login DemoQA user and attach token/expires to the same dict.
    """
    login_body = {
...
def create_and_login_temp_user(account_service: AccountService) -> UserDict:
    """
    Convenience flow to create a random user and login it in one step.
    """
...
def cleanup_demo_user(account_service: AccountService, book_store_service: BookStoreService, user: UserDict) -> None:
    """
    Best-effort cleanup: delete all books and then delete the user.
    Ignore errors (user may already be removed).
    """
